...
import 'body_given_demand.dart';

class MaterialPageGivenDemand extends StatefulWidget {
  const MaterialPageGivenDemand({super.key, required this.demand});
  final Demand demand;


  @override
  State<MaterialPageGivenDemand> createState() => _MaterialPageGivenDemandState();
}

class _MaterialPageGivenDemandState extends State<MaterialPageGivenDemand> {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Padding(
        ...
        child: (isSuccess)?successDonationPage(widget.demand):descriptionDonationPage(),
      ),
    );
  }
                  ...
                  try {
                    if(descriptionEditingController.value.text.isNotEmpty){
                      if(currentUserProvider.profile == KTypeUser.organization) {
                        ...
                        createMaterialDonationByOrganisationToDemand(
                            currentUserProvider.currentOrganization!.organizationId,
                            widget.demand.demandId, materialDonation
                        );
  ...
  Widget successDonationPage(Demand demand) {
    return ListView(
      children: [
        ...
        const SizedBox(height: 10,),
        (demand.user!=null)?AddressCard(
          titleAddress: demand.user!.login!,
          numTel: demand.organization?.numTel,
          address: addresses[0],
          onTapAddress: (){},
        ):AddressCard(
          titleAddress: demand.organization!.login!,
          numTel: demand.organization?.numTel,
          address: addresses[0],
          onTapAddress: (){},
        ),
